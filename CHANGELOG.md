...
### 变更
- 记录窗口去掉顶部「现在在做什么？」内容标题区，常用活动区域直接上移
- 汇总窗口去掉内容区大标题和说明文字，保留窗口标题栏中的汇总标题

## v1.4.3 (2026-06-11)

### 修复
- **修复点击「记录」后菜单栏 app 消失的问题**
  - 去掉 NSPanel 的手动 `release()`，避免 PyObjC/NSWindow 生命周期过度释放导致崩溃
  - 为按钮 handler 保留 Python 引用，避免 Cocoa target 在模态窗口期间被回收
  - 记录弹窗流程增加 `finally`，确保异常后也会释放 `recording_lock`

### 改进
- **重做记录窗口**
  - 更宽松的原生布局，标题、提示、预设区、自定义输入区分层更清晰
  - 自定义输入提示补充逗号、顿号、换行分隔说明
- **重做统计窗口**
  - 今日 / 本周 / 全部汇总改为原生统计面板
  - 新增总记录数、活跃天数、最高频活动
  - 活动分布改为进度条展示，最近记录改为可选择文本的滚动时间线
- 新增统计聚合测试
