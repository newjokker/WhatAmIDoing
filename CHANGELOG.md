...
### 新增
- 新增「导出 CSV」菜单项，可将全部活动记录导出为表格文件，方便用 Numbers、Excel 或数据分析工具打开。

### 变更
- 版本号改为在 `app_version.py` 单点维护，运行时代码、`setup.py`、`Makefile` 和 DMG 构建脚本统一读取同一个版本来源。
- README 更新当前真实的数据存储路径、导出能力和发布维护说明。

## v1.4.15 (2026-06-11)

### 变更
- 配置和活动历史拆分存储：配置写入 `.time_recorder_config.json`，活动历史写入 `.time_recorder_activities.jsonl`
- 新增旧版单文件配置自动迁移，记录活动时改为追加写入，提升大量历史记录下的写入性能

## v1.4.14 (2026-06-11)

### 修复
- 修改每日汇总提醒时间后，会重置当天已弹出标记，使新时间可以继续触发弹窗

## v1.4.13 (2026-06-11)

### 修复
- 修复每日汇总到点后可能未显示却已标记为当天已弹出的问题
- 自动弹出每日汇总前主动激活应用，降低窗口出现在后台的概率

## v1.4.12 (2026-06-11)

### 新增
- 设置中新增「开机自启」开关，通过用户 LaunchAgent 启动已安装的应用

## v1.4.11 (2026-06-11)

### 新增
- 支持每日指定时间自动弹出今日汇总，默认时间为 17:30
- 设置中新增「每日汇总提醒」，可选择常用时间、自定义时间或关闭
...
### 变更
- 重绘应用图标：从单一时钟改为活动时间轴、记录卡片和小钟表组合，强化“记录每时每刻在做什么”的含义
- 更新图标生成脚本，后续构建会稳定生成新版图标

## v1.4.8 (2026-06-11)

### 新增
- 菜单中新增上次提醒时间和下次提醒时间展示
- 新增 `last_reminder_time` 配置字段，区分真实提醒时间和计时基准

## v1.4.7 (2026-06-11)

### 新增
- 新增「导出 JSON」菜单项，可将全部活动记录导出到本地下载目录
- 导出的 JSON 包含应用版本、导出时间、记录总数和完整活动列表

## v1.4.6 (2026-06-11)

### 新增
- 检查更新发现新版时，可直接下载 GitHub Release 中的安装包到本地下载目录
- 下载完成后提示保存路径，并可直接打开下载文件夹
